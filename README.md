...
- `Machine.path(...to)` -> `Boolean|Array<Array<String>>` - The `path` method, similarly to the `will` method takes any number of states and returns either `false` denoting an invalid transtion, or an `Array` of pair tuples, ie. `[ ['A', 'B'], ['B', 'C'] ]` denoting the state changes it would take to achieve the transition, not effecting any change on the machine
